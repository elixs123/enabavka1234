        ...
        <section class="invoice-print mb-1">
            <div class="row">
                <div class="col-8">
                    ...
                    @endif
                    @if($document->isOrder() && !in_array($document->status, ['reversed', 'canceled']))
                                ...
                                <tbody data-document-products>@php $qty = $val = $net = $net_tax = 0; @endphp
                                    ...
                                    <tr class="text-right">
                                        ...
                                        <td>
                                            ...
                                            {{ $item->qty }}
                                            @endif
                                        </td>@php $qty += $item->qty; // getPriceWithoutVat($item->mpc, $document->tax_rate) @endphp
                                        @if(!userIsWarehouse()) @php $price = $document->useMpcPrice() ? $item->mpc : $item->vpc; @endphp
                                        <td>{{ format_price($price) }}</td>@php $discount3 = (($document->isAction() && $document->rAction->isDiscount()) ? ($document->useMpcPrice() ? $item->mpc_discount : $item->vpc_discount) : $document->discount3); $disc = ($document->has_discount) ? ($price - calculateDiscount($price, $document->discount1, $document->discount2, $discount3)) : 0; @endphp
                                        ...
                                        <td><strong>{{ format_price($net_tax_value, 2) }}</strong></td>@php $net_tax += $net_tax_value; @endphp
                                        @endif
                                    </tr>
                                    @endforeach
                                    ...
                                    <tr class="table-active text-right text-bold-600">
                                        ...
                                        <td>&nbsp;</td>
                                        <td>&nbsp;</td>
                                        <td><strong>{{ format_price($net_tax, 2) }}</strong></td>
                                        @endif
                                    </tr>
                                </tbody>
                            </table>
                                ...
                                <table class="table">
                                    <tbody>@php $price_for_discount = $document->useMpcPrice() ? $document->total : $document->subtotal; @endphp
                                        ...
                                        <tr>@php $discount_value3 = getDiscountedValue($document->discount3, $price_for_discount - $discount_value1 - $discount_value2); $discounted = ($discount_value3 > 0); @endphp
                                            ...
                                            <td class="text-right @if($discounted){{ 'text-success' }}@endif">@if($discounted){{ '- ' }}@endif{{ format_price($discount_value3) }} {{ $document->currency }}</td>
                                        </tr>
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    <tbody>
                                        ...
                                        <tr>@php $price_with_vat = getPriceWithVat($price, $document->tax_rate); @endphp
                                        ...
                                        <tr> @php $delivery_cost = calcDeliveryCost($document->delivery_type, $document->rStock->country_id, $price, $document->delivery_cost); @endphp
