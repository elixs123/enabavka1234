...
@section('head_title', $title = trans('action.title'))
            ...
            <div class="form-group breadcrum-right">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right p-0">
                        @can('create-action')
                        <a class="dropdown-item" href="{{ route('action.create', ['callback' => 'actionCreated']) }}" data-toggle="modal" data-target="#form-modal1">{{ trans('action.actions.create') }}</a>
                        @endcan
                        @if(userIsAdmin() || userIsSupervisor())
                        <a class="dropdown-item" href="{{ route('action.stats') }}">{{ trans('action.actions.stats') }}</a>
                        @endif
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col-12 col-sm-8 col-lg-9">
                                        {!! VuexyAdmin::text('keywords', null, ['maxlength' => 100, 'placeholder' => 'Upišite naziv'], trans('skeleton.actions.search')) !!}
                                    </div>
                            ...
                            <div class="table-responsive-lg">
                                <table class="table table-hover mb-0">
                                    <thead class="thead-light">
                                        <tr>
                                            <th>{{ trans('action.data.name') }}</th>
                                            <th>{{ trans('action.data.type_id') }}</th>
                                            <th>{{ trans('action.data.stock') }}</th>
                                            <th>{{ trans('action.data.started_at') }}</th>
                                            <th>{{ trans('action.data.finished_at') }}</th>
                                    ...
                                    <tbody data-ajax-form-body="actions">
...
<script>
    // function actionCreated(response) {
    //     $('#row' + response.data.uid).find('a[data-action-products]:eq(0)').trigger('click');
    // }
</script>
@endsection
